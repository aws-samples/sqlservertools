...
$ReadOverall=' declare @readoverwrite char(1)
                    ...
                    as 
                        ...
                        ,(qs.total_logical_reads)*8/1024.0 AS [Total Logical Reads (MB)],
                            ...
                            qs.total_logical_writes, qs.total_grant_kb  
                                ...
                                (
                                    select  top 10 query_text, totalLogicalread,total_logical_writes,
  ...
  #$ElasticAssessoutput|Export-Csv -Path  $targetfile   -NoTypeInformation -Encoding Unicode 


 }
Function Babelfish {
  #$server='DESKTOP-3C1VC6J!BOBSERVER'
  $script = 'C:\rdstools\babelfish.ps1'
  #& $script -Password $Password -servername $server  -username $login
  Set-Location -Path "C:\rdstools\BabelfishCompass\"
  cmd.exe -/c "C:\rdstools\BabelfishCompass\BabelfishCompass.bat $server c:\RDSTools\out\babelfish\$server\*.sql  -replace -reportoption xref"
...
foreach ($server in $SqlserverEndpoint) {
  ...
  if (Test-SqlConnection $Conn) {
    ...
    if ($babelfish -eq 'Y') {
      ...
      if ($L200Result.dbcount -eq 'Y' -or $L200Result.islinkedserver -eq 'Y' -or $L200Result.issqlTLShipping -eq 'Y' -or $L200Result.isFilestream -eq 'Y' -or $L200Result.isResouceGov -eq 'Y' -or $L200Result.issqlTranRepl -eq 'Y' -or $l200Result.isextendedProc -eq 'Y' -or $L200Result.istsqlendpoint -eq 'Y' -or $L200Result.ispolybase -eq 'Y' -or $L200Result.isfiletable -eq 'Y' -or $L200Result.isbufferpoolextension -eq 'Y' -or $L200Result.isstretchDB -eq 'Y' -or $L200Result.UsedSpaceGB -eq 'Y' -or $L200Result.istrustworthy -eq 'Y' -or $L200Result.Isservertrigger -eq 'Y' -or $L200Result.isRMachineLearning -eq 'Y' -or $L200Result.ISPolicyBased -eq 'Y' -or $L200Result.isdqs -eq 'Y' -or $L200Result.isCLREnabled -eq 'Y')
      { $rdscompatible = 'N' }
